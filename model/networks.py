...
class ResBlock(nn.Module):
    ...
    def forward_denoise(self, h, hs=None, c=None, noise_gain=None, scheduler=None, return_h0=False, timesteps=None):
        h = self(h, hs, c, noise_gain)
        h_0, diffusion_loss = self.feature_denoise(h, scheduler, timesteps)
        h = h_0 if return_h0 else h
        return h, diffusion_loss
...
class Encoder(nn.Module):
    def __init__(self,
        ...
        self.norm_out = nn.InstanceNorm2d(ch_up[-1])
        self.linear_out = nn.Sequential(nn.Linear(ch_up[-1] * current_resolution ^ 2, ch_up[-1]),
                                        nn.SiLU(),
                                        nn.Linear(ch_up[-1], c_in))
    ...
    def forward(self, h):
        ...
        return F.normalize(z), fms

    def sample(self, h):
        B, _, _, _ = h.shape
        z = torch.randn((B, self.c_in))
...
class Decoder(nn.Module):
    ...
    def initalize_feature_denoiser(self):
        parameters = []
        for idx, encoder_block in enumerate(self.encoder):
            parameters += encoder_block.block.initalize_feature_denoiser()
        parameters += self.encoder_out.initalize_feature_denoiser()
        for idx, decoder_block in enumerate(self.decoder):
            parameters += decoder_block.block.initalize_feature_denoiser()
        parameters += self.decoder_out.initalize_feature_denoiser()
        return parameters

    def forward_denoise(self, h, c=None, scheduler=None, return_h0=False, timesteps=None):
        ...
        diffusion_loss = 0
        for idx, encoder_block in enumerate(self.encoder):
            h, dl = encoder_block.block.forward_denoise(h, c=c, scheduler=scheduler, return_h0=return_h0,
                                                        timesteps=timesteps)
            hs[current_resolution] = h
            h = encoder_block.resample(h)
            current_resolution /= 2
        ...
        h, dl = self.encoder_out.forward_denoise(h, c=c, scheduler=scheduler, return_h0=return_h0,
                                                 timesteps=timesteps)
        hs[current_resolution] = h
        ...
        for idx, decoder_block in enumerate(self.decoder):
            ...
            h, dl = decoder_block.block.forward_denoise(h, c=c, hs=skip, scheduler=scheduler, return_h0=return_h0,
                                                        timesteps=timesteps)
        ...
        h, dl = self.decoder_out.forward_denoise(h, c=c, hs=skip, scheduler=scheduler, return_h0=return_h0,
                                                 timesteps=timesteps)
        ...
        if self.mask_out is not None:
            m = torch.sigmoid(self.mask_out(h))
            return i, m, diffusion_loss
        else:
            return i, diffusion_loss
...
class DiscriminatorV2(nn.Module):
    def __init__(self,
                 ...
                 kv_compress: Dict[int, int] = {8: 1, 16: 2, 32: None, 64: None, 128: None, 256: None},
                 skip_resolutions: Tuple[int] = (16, 32, 64, 128, 256),
                 resample_type: str = 'conv_interpolate_up',
                 adaptive_type: str = 'scale_shift_gate',
                 skip_type: str = 'addition',
        ...
        self.encoder_out = ResBlock(ch[-1], ch[-1], c_in,
                                    attention=current_resolution in attention_resolutions,
                                    kv_compress=kv_compress[current_resolution],
                                    adaptive_type=adaptive_type)

        self.encoder_discriminator_head = nn.Sequential(nn.Conv2d(ch[-1], ch[-1], 4, 1),
                                                        nn.SiLU())
    ...
    def forward(self, h, c, train_d=True):
        ...
        h = self.encoder_out(h, c=c)
        h = self.encoder_discriminator_head(h)

        direction = F.normalize(self.fc_w, dim=1)
        scale = torch.norm(self.fc_w, dim=1).unsqueeze(1)

        h = h.permute(0, 2, 3, 1) * scale

        if train_d:
            out_fun = (h * direction.detach()).sum(dim=-1).unsqueeze(1)
            out_dir = (h.detach() * direction).sum(dim=-1).unsqueeze(1)

            return out_fun, out_dir

        else:
            return (h * direction).sum(dim=-1).unsqueeze(1)
